...
const ReportsPage = () => {
  ...
  const defaultReportType: ReportType = (user?.role === 1 || user?.role === 2 || user?.role === 3) ? "teacher_groups_comparison" : "student_evolution";
  ...
  useEffect(() => {
    if ((user?.role === 1 || user?.role === 2 || user?.role === 3) && token) {
